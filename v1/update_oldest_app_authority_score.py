...
def save_oldest_app_authority_score():
  url = execute_select_one("select url from url_app_appl order by updated_at desc limit 1")
  ...
  if not app_score_log:
    ...
    target_app_score_dict = execute_select_one(f"select url_to, count(url_from) as score from url_app_appl_suggested group by url_to where url = '{url}'")
